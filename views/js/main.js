...
/* Instead of a separate function for each ingredient, a single function was created 
and calculation happens based on the input passed*/
...
var itemGenerator = function(item) {
    switch (item) {
        case "meat":
        ...
        default:
            console.log("bug in itemGenerator");
    }
};
...
var makeRandomPizza = function() {
    ...
    for (var i = 0; i < numberOfMeats; i++) {
        pizza = pizza + ingredientItemizer(itemGenerator("meat"));
    }

    for (var j = 0; j < numberOfNonMeats; j++) {
        pizza = pizza + ingredientItemizer(itemGenerator("nonmeats"));
    }

    for (var k = 0; k < numberOfCheeses; k++) {
        pizza = pizza + ingredientItemizer(itemGenerator("cheese"));
    }

    pizza = pizza + ingredientItemizer(itemGenerator("sauce"));
    pizza = pizza + ingredientItemizer(itemGenerator("crust"));
...
/* commented the code to check for scroll and trigger updatePositions function. 
Instead call the updatePositions function only after reaching certain Scroll Limit  */
...
document.addEventListener('DOMContentLoaded', function() {
    //var cols = 8;
    var s = 256;
    /*Modified the number of moving pizza based on the screen height and width*/
    for (var j = 0; j < screen.height; j += s) {
        for (var i = 0; i < (screen.width); i += s) {
